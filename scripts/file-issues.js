  ...
  for (const f of rawFindings) {
    ...
    if (!byFp.has(id)) {
      byFp.set(id, {
        ...
        source: (f.Source || f.source || 'gitleaks'),
        extra: f.Extra || f.extra || null,
...
function severityOf(finding, repoPriority) {
  // Prefer the scanner-assigned severity if present.
  // Escalate secret findings on critical-priority repos to critical regardless.
  const scannerSev = (finding.Severity || finding.severity || '').toLowerCase();
  const source = (finding.Source || finding.source || '').toLowerCase();

  // Secret findings on a critical-priority repo are always critical
  if (repoPriority === 'critical' && (source === 'gitleaks' || source === '')) {
    return 'critical';
  }

  if (['critical', 'high', 'medium', 'low'].includes(scannerSev)) {
    return scannerSev;
  }

  // Back-compat fallback: Phase 1 Gitleaks findings didn't have Severity field
...
// ---------------------------------------------------------------------------
function renderIssueBody({ repo, repoPriority, activeFindings, dismissedFindings, runUrl, generatedAt }) {
  const lines = [];
  ...
  if (Object.keys(bySource).length > 0) {
    lines.push('**By scanner:** ' + Object.entries(bySource)
      .sort(([, a], [, b]) => b - a)
      .map(([src, n]) => `${src}: ${n}`)
      .join('  ·  '));
    lines.push('');
  ...
  if (activeFindings.length === 0) {
    ...
    for (const sev of ['critical', 'high', 'medium', 'low']) {
      ...
      // Compact mode: when a severity has many findings (common with PHPCS),
      // render as a single collapsed table rather than N detail blocks.
      // Threshold: >15 findings in this severity → compact table.
      const COMPACT_THRESHOLD = 15;
      if (forSev.length > COMPACT_THRESHOLD) {
        // Group compactly by source so PHPCS noise doesn't bury other signals
        const bySource = {};
        for (const f of forSev) {
          if (!bySource[f.source]) bySource[f.source] = [];
          bySource[f.source].push(f);
        }
        for (const [src, findings] of Object.entries(bySource)) {
          lines.push(`<details><summary><strong>${src}</strong> — ${findings.length} findings</summary>`);
          lines.push('');
          lines.push('| ID | Rule | File | Line | Message |');
          lines.push('|---|---|---|---|---|');
          // Cap per-source at 100 entries to stay within GitHub's 65KB body limit
          const shown = findings.slice(0, 100);
          for (const f of shown) {
            const rule = String(f.ruleId).slice(0, 40);
            const file = String(f.file).slice(0, 60);
            const msg = String(f.match || '').slice(0, 80).replace(/\|/g, '\\|').replace(/\n/g, ' ');
            lines.push(`| \`${f.id}\` | ${rule} | \`${file}\` | ${f.line} | ${msg} |`);
          }
          if (findings.length > shown.length) {
            lines.push('');
            lines.push(`_${findings.length - shown.length} additional ${src} findings truncated — see \`reports/${repo}/${src}.json\` for full list._`);
          }
          lines.push('');
          lines.push('</details>');
          lines.push('');
      ...
      // Detailed mode for small counts (critical/high typically)
      for (const f of forSev) {
        lines.push(`#### \`${f.id}\` — ${f.ruleId}  <sub>[${f.source}]</sub>`);
        lines.push('');
...
function explainRule(ruleId, source) {
  // Secret-specific rules (Gitleaks + our custom patterns)
  const gitleaksMap = {
  ...
  if (source === 'gitleaks' && gitleaksMap[ruleId]) return gitleaksMap[ruleId];

  // Source-specific generic explanations
  switch (source) {
    case 'gitleaks':
      return 'A potential secret was detected by Gitleaks. Review the match and decide whether it is a real credential.';
    case 'composer-audit':
      return 'A PHP dependency has a known published security advisory. The vulnerability details and fixed version are in the finding\'s Match field and the advisory link.';
    case 'npm-audit':
      return 'An npm dependency has a known published security advisory. `npm audit` reports a vulnerability in this package or one of its transitive dependencies.';
    case 'osv-scanner':
      return 'The OSV (Open Source Vulnerabilities) database matched a vulnerability in one of this repo\'s dependencies. Cross-ecosystem coverage — may flag issues that composer audit or npm audit miss.';
    case 'phpcs':
      return 'A PHP Code Sniffer rule was violated. This is a code-quality or WordPress-convention finding. Not a security issue by default, but many rules (missing escaping, capability checks, nonce verification) map directly to vulnerability patterns.';
    case 'semgrep':
      return 'Semgrep matched a pattern defined in one of its rule packs. Could be a security smell (SQLi, XSS, hardcoded crypto), an OWASP Top 10 issue, or an anti-pattern in the framework you\'re using. Severity indicates how likely the finding is to be exploitable.';
    default:
      return 'A finding was reported by a scanner. Review the File/Line and Detail fields to understand what pattern was matched.';
  }
}
...
function fixSteps(finding) {
  ...
  switch (source) {
    ...
    case 'osv-scanner':
      return [
        `Review the advisory: ${finding.extra?.link || finding.extra?.url || finding.extra?.summary || 'check the finding detail'}`,
        'Check if a fixed version is available. If yes: upgrade the package.',
        'If no fix is available: assess whether the vulnerable code path is actually reachable in your app.',
        'If reachable and unpatched: consider pinning to an alternative, or apply a workaround (input validation, disabled feature flag).',
        'Regenerate the lockfile and commit the update.',
        'Re-run the scan to verify the finding clears.',
      ];
    case 'phpcs':
      return [
        `Open \`${finding.file}\` at line ${finding.line}`,
        `Read the rule description: \`${finding.ruleId}\``,
        'If legitimate: fix the code. Many PHPCS findings auto-fix via `phpcbf` (the companion tool).',
        'If the rule is wrong for this codebase: add a rule-specific exclusion in `phpcs-ruleset.xml` and dismiss.',
        `Consider fixing at PR time rather than as a batch — ${finding.ruleId} will appear in every file that has the same issue.`,
      ];
    case 'semgrep':
      return [
        `Open \`${finding.file}\` at line ${finding.line}`,
        `Review the pattern. Rule URL: ${finding.extra?.ruleUrl || 'semgrep.dev/r/' + finding.ruleId}`,
        'If it\'s a real vulnerability (SQLi, XSS, auth bypass): fix the code. Rotate credentials if exposed.',
        'If it\'s a false positive for this context: label the issue `false-positive` and add a `// nosemgrep: <rule-id>` comment at the line to suppress future occurrences.',
        'If the pattern is always-safe in your codebase: open a PR against `configs/semgrep.yml` to disable the rule globally.',
      ];
    case 'gitleaks':
    default:
      return [
