 ...
 *
 * Reads:
 *   - targets.json  (produced from targets.yml by the workflow via yq)
 ...
 *
 * Run locally:
 *   yq -o=json targets.yml > targets.json && node scripts/aggregate.js
 */
...
function statusCell(result, priority) {
  ...
  // Critical repos get flagged red on any finding; others scale by count
...
function generate() {
  ...
  // Sort: critical first, then by priority order
  ...
  lines.push('');
  lines.push('- 🔴 **Critical repos** (like `kw-security-plugin`) are flagged red on *any* finding — treat every finding as a live credential until proven otherwise.');
  ...
  lines.push('- ⚪ **Not scanned** = repo is in `targets.yml` but no report has run yet.');
  lines.push('');
  ...
  lines.push('## Roadmap');
  lines.push('');
  lines.push('- [x] **Phase 1** — Gitleaks secret scanning across full history (this)');
  lines.push('- [ ] **Phase 2** — Stack-aware scanners: `composer audit`, `npm audit`, PHPCS+WPCS, Semgrep');
  lines.push('- [ ] **Phase 3** — Auto-open/update `[health-check]` issues in target repos');
